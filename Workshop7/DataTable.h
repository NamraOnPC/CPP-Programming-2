

#ifndef _DATA_TABLE_H_
#define _DATA_TABLE_H_

#include <iostream>
#include <functional>
#include <numeric>
#include <algorithm>
#include <vector>
#include <cmath>
using namespace std;

namespace w7 {

	template <typename T>
	class DataTable {
		
		std::vector<T> _X;
		
		std::vector<T> _Y;

		int _FW;
		
		int _ND;

	public:

		DataTable(std::ifstream& in, int FW, int ND)
			: _FW(FW), _ND(ND) {
			
			T x, y;
			
			while (!in.eof()) {

				in >> x >> y;
				
				_X.push_back(x);
				
				_Y.push_back(y);
			
			}
		
		}

		
		//returns the mean value of the dependent coordinate

		T mean() const {
			
			T sum = std::accumulate(_Y.begin(), _Y.end(), (T)0);
			
			T num = _Y.size();

			return sum / num;
		
		}

		
		//	returns the standard deviation of the dependent coordinates
		
		T sigma() const {
			
			T mean_ = mean();
			
			T num = _Y.size();

			std::vector<T> diff(num);
			
			std::transform(_Y.begin(), _Y.end(), diff.begin(), std::bind2nd(std::minus<T>(), mean_));

			T sum_sq = std::inner_product(diff.begin(), diff.end(), diff.begin(), (T)0);
			
			return (T)std::sqrt(sum_sq / (num - 1));
		
		}

		
		//	returns the median value of the dependent coordinate
		
		T median() const {
			
			std::vector<T> sort_ = this->_Y;
			
			std::sort(sort_.begin(), sort_.end());
			
			T num = _Y.size();

			return sort_[num / 2];
		
		}

		
		//	returns the slope and intercept for the data set
		
		void regression(T& slope, T& y_intercept) const {
			
			T sum_xy = std::inner_product(_X.begin(), _X.end(), _Y.begin(), (T)0);
			
			T sum_x = std::accumulate(_X.begin(), _X.end(), (T)0);
			
			T sum_y = std::accumulate(_Y.begin(), _Y.end(), (T)0);
			
			T sum_sq_x = std::inner_product(_X.begin(), _X.end(), _X.begin(), (T)0);
			
			T num = _Y.size();

			slope = (num * sum_xy - sum_x * sum_y) / (num * sum_sq_x - sum_x * sum_x);
			
			y_intercept = (sum_y - slope * sum_x) / num;
		
		}

		
		//	displays the data pairs as shown below
		
		void display(std::ostream& os) const {
			
			os << std::setprecision(_ND) << std::fixed;
			
			os << std::setw(_FW) << 'x' << std::setw(_FW) << 'y' << std::endl;

			for (int i = 0; i < _X.size(); i++)
			
				os << std::setw(_FW) << _X[i] << std::setw(_FW) << _Y[i] << std::endl;
		
		}

		
		//	inserts the data generated by display() into the output stream
		
		friend std::ostream& operator<<(std::ostream& os, const DataTable& D){
			
			D.display(os);
			
			return os;
		
		}
	
	};

}



#endif
